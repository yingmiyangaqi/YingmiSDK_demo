...
- (IBAction)login:(id)sender {
    ...
    d = [HttpClient doPost:@"https://csdk-test.yingmi.cn/ymb/v1/user/login" parameters:@{@"name":name,@"password":password}];
...
- (IBAction)token:(id)sender {
    ...
    d = [HttpClient doGet:@"https://csdk-test.yingmi.cn/ymb/v1/user/token" parameters:@{@"access_token":token}];
    ...
    [d then:^id(id resultObject) {
        ...
        if(dict){
            ...
            if(!weakSelf.isInit){
                ...
                [YingmiCsdk initSDKWithConfig:@{@"brokerId":@"123456",@"token":access_token} completeBlock:^(id err, id data) {
                    if(err){
                        weakSelf.isInit = NO;
                    }else{
                        weakSelf.isInit = YES;
                ...
                [YingmiCsdk setToken:access_token];
            }
        }
